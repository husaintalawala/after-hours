...
export default function Filmstrip({ photos, videos, chapterTitle }: FilmstripProps) {
  ...
  const handleTap = useCallback((i: number) => {
    if (!audioUnlocked) { audioUnlocked = true; setUnlocked(true) }
  ...
  return (
    <div className="mt-5 -mx-6 md:-mx-8">
      <div className="flex items-center gap-3 px-6 md:px-8 mb-3">
        ...
        <span className="text-[10px] font-mono text-[#424245]">{media.length}</span>
      </div>
      ...
      <div className="text-center mt-1">
        <span className="text-[9px] font-mono text-[#424245]">{Math.max(focusIndex + 1, 1)} / {media.length}</span>
      </div>
    </div>
  )
}
