...
import { createTripFromItinerary } from "@/lib/drift/createTripFromItinerary"
 ...
 * days as sections, places as rows, and one button that turns the lot into a
 * real trip. Without this the block is stripped and thrown away, which is a
 * worse answer than never asking for it — the reader gets a paragraph where
 * they were promised a plan.
 *
...
export default function ItineraryCard({ itin }: { itin: ChatItinerary }) {
  ...
  const places = itin.days.reduce((n, d) => n + d.places.length, 0)

  async function create() {
    if (busy) return
    setBusy(true)
    setError(null)
    const res = await createTripFromItinerary(itin)
    if ("error" in res) {
      setError(res.error)
      setBusy(false)
      return
    ...
    <section className="mt-3 overflow-hidden rounded-[18px] border border-aurora-border bg-aurora-glass">
      <header className="border-b border-aurora-border px-4 py-3.5">
        ...
        <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.12em] text-aurora-ink3">
          {itin.days.length} {itin.days.length === 1 ? "day" : "days"} &middot; {places}{" "}
            ...
            <ul className="mt-2 space-y-2">
              {day.places.map((p) => (
                <li key={p.name} className="flex gap-2.5">
                  <span
                    aria-hidden
                    className="mt-[7px] h-1.5 w-1.5 shrink-0 rounded-full bg-aurora-teal/70"
                  />
